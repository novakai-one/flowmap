...
For large codebases, the bundle is assembled from a root file plus one fragment per folder:

- **`docs/flowmap/root.mmd`** — the global namespace: container nodes (one per folder/unit), shared nodes (stores, types, services), and cross-folder edges. Every id defined in `root.mmd` is global.
- **One `flowmap.mmd` per folder**, self-rooted with `%% root <containerId>` where `<containerId>` is a node defined in `root.mmd` (the join key). Any id not global is private and gets namespaced `<containerId>__<id>` at merge time.
- `--dir src` tells the bundler to find every `flowmap.mmd` under `src/` recursively.

The bundler merges them, drops geometry (`%% fm` / `%% edge` lines — Tidy re-lays), and emits one valid `.mmd`.

You don't have to use fragments. You can author one `.mmd` by hand (or have an LLM emit it) per `SYNTAX_README.md` and Load that. This repo itself uses a single flat `root.mmd` (no per-folder fragments) because `src/` is flat.
...
| `src/**/flowmap.mmd` — one fragment per folder | Nothing — your source is never copied into the Flowmap repo | The loaded diagram + prefs (autosaved to localStorage) |
