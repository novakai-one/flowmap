...
export function initNodes(ctx, selection, camera) {
    const { state } = ctx;
    ...
    function alignNodes(mode) {
        ...
        if (mode === 'right')
            ns.forEach((n) => { n.x = maxR - n.w; });
        if (mode === 'top')
            ns.forEach((n) => { n.y = minY; });
        if (mode === 'bottom')
            ns.forEach((n) => { n.y = maxB - n.h; });
        if (mode === 'cx')
            ns.forEach((n) => { n.x = cxAll - n.w / 2; });
        if (mode === 'cy')
            ns.forEach((n) => { n.y = cyAll - n.h / 2; });
        if (mode === 'dh') {
