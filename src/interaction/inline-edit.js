...
export function initInlineEdit(ctx, camera, nodes) {
    ...
    function beginEdit(id) {
        ...
        const selo = window.getSelection();
        ...
        const key = (ev) => {
            if (ev.key === 'Enter' && !ev.shiftKey) {
                ...
                lab.textContent = state.nodes[id].label;
                done = true;
                runtime.editingId = null;
                el.classList.remove('editing');
                lab.removeAttribute('contenteditable');
    ...
    stage.addEventListener('dblclick', (e) => {
        ...
        const card = e.target.closest('.fmcard');
        if (card) {
            const host = card.closest('.node');
            const id = host?.dataset.id;
            if (id)
                openFrontmatterEditor(id);
            e.stopPropagation();
            return;
        }
    ...
    function openFrontmatterEditor(id) {
        if (!state.nodes[id])
            return;
        state.sel.clear();
        state.sel.add(id);
        ...
        setTimeout(() => {
            const name = document.getElementById('fmName');
            if (name) {
                name.focus();
                name.select();
            }
