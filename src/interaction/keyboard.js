...
export function initKeyboard(ctx, deps) {
    const { stage } = ctx.dom;
    const { state, cam } = ctx;
    ...
    window.addEventListener('keydown', (e) => {
        ...
        if (mod && e.key.toLowerCase() === 'v') {
            e.preventDefault();
            clipboard.pasteClip(ctx.lastMouseWorld);
            return;
        }
        if (mod && e.key.toLowerCase() === 'd') {
            e.preventDefault();
            clipboard.duplicateSel();
            return;
        }
        ...
        if (e.key === 'Enter' && state.sel.size === 1) {
            e.preventDefault();
            inlineEdit.beginEdit([...state.sel][0]);
            return;
        }
        if (e.key.toLowerCase() === 'l') {
            pointer.setLinkMode(!pointer.isLinkMode());
            return;
        }
