 ...
 */
export function buildFmCard(fm: Frontmatter): HTMLElement {
  const card = document.createElement('div');
  card.className = 'fmcard';
  const clean = (items: string[]): string[] => items.filter((s) => s.trim());
  const row = (key: string, items: string[]): string =>
    clean(items).length
      ...
      : '';
  let html = '';
  if (fm.name.trim()) html += `<div class="fmrow"><span class="fmkey">name</span><span class="fmval">${esc(fm.name)}</span></div>`;
