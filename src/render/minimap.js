...
export function initMinimap(ctx, camera) {
    const { stage } = ctx.dom;
    const cam = ctx.cam;
    let mmFit = null;
    ...
    if (mm) {
        ...
        mm.addEventListener('pointermove', (e) => { if (dragging)
            minimapTo(e.clientX, e.clientY); });
        ...
        mm.addEventListener('pointerleave', (e) => { if (dragging && e.buttons === 0)
            endDrag(e); });
    }
    return { drawMinimap };
}
