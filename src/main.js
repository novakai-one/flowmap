...
document.querySelectorAll('#shapeTools .tool').forEach((b) => {
    b.onclick = () => { pointer.setLinkMode(false); nodes.addNode(b.dataset.shape); };
});
...
$('clearAll').onclick = () => {
    if (!Object.keys(ctx.state.nodes).length)
        return;
...
$('helpOverlay').onclick = (e) => { if (e.target === $('helpOverlay'))
