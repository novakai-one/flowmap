/* =====================================================================
   ...
   ===================================================================== */
import { THEMES, THEME_VARS, FONTS } from '../core/config';
import { savePrefs } from '../core/persistence';
export function initTheming(ctx) {
    const { stage } = ctx.dom;
    function applyTheme(key, doRender = true) {
        const t = THEMES[key];
        if (!t)
            return;
    ...
    function applyCanvasPrefs() {
        ...
        if (mm)
            mm.classList.toggle('hidden', !ctx.prefs.map);
        ctx.snap = ctx.prefs.snap;
        const sb = document.getElementById('snapBtn');
        if (sb)
            sb.classList.toggle('active', ctx.snap);
