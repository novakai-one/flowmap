...
export function initInspector(ctx, nodes, selection) {
    const { state } = ctx;
    ...
    function renderInspector() {
        ...
        if (state.selEdge) {
            renderEdgeInspector(inspBody);
            updateStatus();
            return;
        }
        if (state.sel.size > 1) {
            renderMultiInspector(inspBody);
            updateStatus();
            return;
        }
    ...
    function updateStatus() {
        ...
        if (state.sel.size)
            s += ` · ${state.sel.size} selected`;
        statusEl.textContent = s;
    }
    function renderSingleInspector(body) {
        const n = state.nodes[[...state.sel][0]];
        body.innerHTML = `
      <div class="field"><label>Label</label><input id="iLabel" value="${esc(n.label)}"></div>
      <div class="field"><label>Shape</label><select id="iShape">${SHAPES.map((s) => `<option value="${s}" ${s === n.shape ? 'selected' : ''}>${s}</option>`).join('')}</select></div>
      <div class="field"><label>Fill</label><div class="swatches" id="iSw">${PALETTE.map((c, i) => `<div class="sw ${c === n.color ? 'on' : ''}" data-c="${c}" title="${PALETTE_NAMES[i]}" style="background:${c}"></div>`).join('')}</div></div>
      ...
      <button class="filebtn danger" id="iDel">Delete node</button>`;
        const labelInp = $('iLabel');
        labelInp.oninput = (e) => { n.label = e.target.value; ctx.hooks.render(); ctx.hooks.sync(); };
        ...
        $('iSw').querySelectorAll('.sw').forEach((sw) => {
            sw.onclick = () => {
                n.color = sw.dataset.c === 'null' ? null : sw.dataset.c;
                ctx.hooks.render();
                ctx.hooks.sync();
                renderInspector();
                ctx.hooks.pushHistory();
        ...
        $('iDel').onclick = () => nodes.deleteSelection();
    ...
    function renderMultiInspector(body) {
      ...
      <div class="field"><div class="swatches" id="mSw">${PALETTE.map((c, i) => `<div class="sw" data-c="${c}" title="${PALETTE_NAMES[i]}" style="background:${c}"></div>`).join('')}</div></div>
      ...
      <button class="filebtn danger" id="mDel">Delete ${state.sel.size} nodes</button>`;
        body.querySelectorAll('[data-al]').forEach((b) => {
            b.onclick = () => { nodes.alignNodes(b.dataset.al); ctx.hooks.pushHistory(); };
        });
        $('mSw').querySelectorAll('.sw').forEach((sw) => {
            sw.onclick = () => {
                const c = sw.dataset.c === 'null' ? null : sw.dataset.c;
                state.sel.forEach((id) => {
                    if (state.nodes[id].shape !== 'group' && state.nodes[id].shape !== 'note')
                        state.nodes[id].color = c;
                });
                ctx.hooks.render();
                ctx.hooks.sync();
                ctx.hooks.pushHistory();
        ...
        $('mGroup').onclick = () => nodes.wrapInGroup();
        $('mDel').onclick = () => nodes.deleteSelection();
    }
    function renderEdgeInspector(body) {
      ...
      <button class="filebtn danger" id="eDel">Delete edge</button>`;
        const labelInp = $('eLabel');
        labelInp.oninput = (ev) => { e.label = ev.target.value; ctx.hooks.render(); ctx.hooks.sync(); };
        labelInp.onchange = () => ctx.hooks.pushHistory();
        $('eStyle').onchange = (ev) => { e.style = ev.target.value; ctx.hooks.render(); ctx.hooks.sync(); ctx.hooks.pushHistory(); };
        $('eRoute').onchange = (ev) => { e.routing = ev.target.value; ctx.hooks.render(); ctx.hooks.sync(); ctx.hooks.pushHistory(); };
        $('eFlip').onclick = () => { const t = e.from; e.from = e.to; e.to = t; ctx.hooks.render(); ctx.hooks.sync(); ctx.hooks.pushHistory(); };
        $('eDel').onclick = () => { state.edges = state.edges.filter((x) => x.id !== e.id); selection.clearSel(); ctx.hooks.sync(); ctx.hooks.pushHistory(); };
    }
