...
- 2026-07-02 · stage 3 · 9a30ccb · gate green, tsc clean, 166/166; human-verified in browser (explore mode): wire click → select + inspector (endpoints/kind/direction/carries), re-click deselects, prior wire/node deselected on new wire click, hover pre-light, canvas click-without-drag deselects (3px drag threshold, fixed post-verify). KNOWN GAPS carried forward: (a) stage-mode wire click is a noop despite hit paths being appended to sWiresEl — investigate (suspect stagelayer stacking/pointer-events), wires must be first-class in stage too; (b) stage-mode background click no-op (accepted for now — ← explore exits).
- 2026-07-02 · stage 4 · 53c32a6 · gate + ship green, tsc clean, 166/166; Chrome-MCP-verified in browser: group header click → select + ring + full group inspector (GROUP chip · crumbs · derived role "5 modules" · contains(5) with kind tags · uses→/used-by aggregated per foreign top group with weights — sibling groups stay themselves); chevron folds WITHOUT touching selection; collapsed group card selects (no expand, no stage), corner ⤢/dblclick expands; contains/conns rows travel; blast + group = subtree-seeded ripple, members stay lit, no full-canvas dim; regressions held (module card still stages, wire click still selects, ← explore, Esc chain, layer toggles). Two new gated keystones: unfold.ts#selectGroup, unfold.ts#groupConns. NOTES: (a) expanded-module-header select shares the exact groupEl→selectGroup path (verified on hier groups; this diagram has no drilled modules to click live); (b) dblclick-on-header fires select→deselect before folding (pre-existing cardEl pattern, kept for consistency); (c) stage-mode wire-click noop still carried from stage 3. Group selection deliberately does NOT stage — that is U8/stage 5, design-first.
