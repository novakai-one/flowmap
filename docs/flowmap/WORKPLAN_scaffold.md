...
Walks a TS project with no existing fragments, groups exported symbols by folder, emits draft fragments + draft root.mmd with all mechanical lines pre-filled (src, kind, name, accepts, returns with real types, dotted import edges). Output fails `flowmap-lint` by design — it is a starting point.
...
node tools/buildspec/scaffold.mjs --backfill src/core/camera/camera.flowmap.mmd --tsconfig tsconfig.json --dry

# Apply to one fragment
node tools/buildspec/scaffold.mjs --backfill src/core/camera/camera.flowmap.mmd --tsconfig tsconfig.json

# Verify: camera fragment now has i0 lines for applyCam, toWorld, etc.
grep "i0\." src/core/camera/camera.flowmap.mmd

# Re-run backfill — should be a no-op
node tools/buildspec/scaffold.mjs --backfill src/core/camera/camera.flowmap.mmd --tsconfig tsconfig.json --dry

# Run the gate — warnings for camera nodes should be gone
npm run flowmap:ship && npm run spec:gate 2>&1 | grep "camera__" | wc -l
# Should be 0 (was 6)
```

**Apply to all fragments:**
```bash
for f in src/*/*/*.flowmap.mmd; do
...
```bash
npm run flowmap:ship          # must still PASS (lint, validate)
...
**Risk:** Medium. The insertion logic is the trickiest part. If lines are inserted in the wrong place, the fragment may become invalid. Mitigation: `--dry` flag, test on one fragment first, verify with `flowmap:ship` after each.
...
│        c. Write to <outDir>/<folder>.flowmap.mmd
...
5. **Output location** — fragments go in their respective source folders (e.g., `src/core/camera/camera.flowmap.mmd`). root.mmd goes to `docs/flowmap/root.mmd`. Use `--out` to override root.mmd location.
...
mkdir -p /tmp/flowmap-backup
cp docs/flowmap/root.mmd /tmp/flowmap-backup/
cp src/*/*/*.flowmap.mmd /tmp/flowmap-backup/ 2>/dev/null

# Delete all mmd files
rm docs/flowmap/root.mmd src/*/*/*.flowmap.mmd

# Run init
node tools/buildspec/scaffold.mjs --init --tsconfig tsconfig.json --src src --out docs/flowmap

# Check output
ls -la docs/flowmap/root.mmd src/*/*/*.flowmap.mmd

# Verify draft has real types (not arg0: unknown)
grep "i0\.accepts=" src/core/camera/camera.flowmap.mmd
# Should show: i0.accepts=ctx: AppContext (real type, not arg0: unknown)

# Verify draft FAILS lint (expected — it's a file-mirror)
npm run flowmap:bundle 2>&1 | tail -5
node tools/flowmap/flowmap-lint.mjs docs/flowmap/_bundle.mmd
# Should FAIL (FLAT or LOOSE-BAG) — this is expected

# Restore originals
cp /tmp/flowmap-backup/root.mmd docs/flowmap/root.mmd
cp /tmp/flowmap-backup/*.flowmap.mmd src/*/*/*.flowmap.mmd 2>/dev/null
# (may need manual copy per folder)
```

**Lines of code:** ~200 lines (extends the ~120 from Step 2; total file ~320 lines).

**Risk:** Low-medium. The output is a draft that is expected to fail lint. The main risk is producing invalid fragment syntax that the bundler can't parse. Mitigation: verify with `flowmap:bundle` after generation.
...
"flowmap:init": "node tools/buildspec/scaffold.mjs --init --tsconfig tsconfig.json --src src --out docs/flowmap"
```

Note: `flowmap:backfill` uses a shell loop because scaffold processes one fragment at a time. Alternatively, add a `--dir` flag to scaffold that processes all fragments in a directory.
...
**Files:** `tools/BUILD_FLOWMAP.md`, `README.md`

Add a "Step 0: Bootstrap" section to `BUILD_FLOWMAP.md`:
...
2. The draft FAILS `flowmap-lint` by design (it is a file-mirror). You MUST do
   the architectural work from Step 1 onward before it will pass.

3. After authoring your fragments (Step 1–3), run `npm run flowmap:backfill` to
...
npx tsc --noEmit                               # PASS
npm run flowmap:ship                           # PASS (lint + validate + bodies)
...
npm run flowmap:backfill                       # should report 0 added, 48 skipped
...
node tools/flowmap/bundle.mjs --root /tmp/init-test/root.mmd --dir /tmp/init-test 2>&1 | head -5
# Should produce a parseable bundle (will fail lint, but should parse)

# 5. Init produces real types (not arg0: unknown)
grep "i0\.accepts=" /tmp/init-test/src/core/camera/camera.flowmap.mmd
...
1. **Do NOT make --init output pass flowmap-lint.** The draft is a file-mirror by construction. Making it pass lint would require architectural decisions (sections, spine, pruning) that defeat the purpose of automation.
...
4. **Do NOT auto-run backfill in `flowmap:ship`.** Backfill mutates source files (fragments). It should be an explicit step the user runs, not a side-effect of shipping.
...
|---|---|---|---|
| Backfill inserts lines at wrong position | Medium | High (corrupts fragment) | `--dry` flag, test on 1 fragment, verify with `flowmap:ship` |
...
| Init produces unparseable fragment syntax | Low | Medium (draft is useless) | Verify with `flowmap:bundle` after generation |
