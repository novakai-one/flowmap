...
Phase AUD1 (`docs/flowmap/audit/WORK_ORDER.md`). Classify every `CLM-*` from `00-inventory.md` as
**GATE**, **CONVENTION**, or **PROSE**. Per GATE: exact trigger + bypass surface. No fixes. Status:
`npm run flowmap:audit`.
...
- CI = `.github/workflows/spec-gate.yml` jobs `buildspec-tests` + `flowmap-drift`.
...
- **Not in CI or any hook:** the quiz, `frag-check`, `status`, `trust-report`, and the entire
  tooling-map chain (`flowmap:tooling:*`, `tooling-coverage`, `tooling-map.test`). `deploy.yml` runs
  no flowmap gates.
...
grep -rnE "quiz|frag-check|tooling|flowmap:status|flowmap:trust|flowmap:onboard" .github/ .claude/
...
for n in $(grep -oE 'CLM-[0-9]+' docs/flowmap/audit/00-inventory.md | sort -u); do \
  c=$(grep -c "\b$n\b" docs/flowmap/audit/01-claims.md); echo "$n $c"; done
```
...
| CLM-003 | GATE | CI `flowmap:gate` (flowmap-drift job) | ts-morph `allowJs:false` → `.mjs` tooling ungated; 1 known prose type hole; only fires on trigger paths |
| CLM-004 | GATE | CI `flowmap:acceptance -- --plan public/plan.json` | acceptance only covers PURE exported fns; ctx/DOM code needs factor-to-pure (E2 gap); scoped to `public/plan.json` |
| CLM-005 | GATE | CI `flowmap:roadmap:audit` | only lints `CLAUDE.md`; narrow 2-regex pattern (see CLM-007) |
...
| CLM-007 | GATE | CI `flowmap:roadmap:audit` (exit 1) | matches only `**State:**` / `state: ❌⚠️✅…`; any other status phrasing (table cell, HTML, "shipped") evades; only `CLAUDE.md` |
| CLM-008 | GATE | CI `flowmap:gate` | as CLM-003 |
| CLM-009 | GATE | CI `spec:conformance` (parser-conformance.test) | corpus-bounded — agreement only on cases in the corpus |
| CLM-010 | GATE | CI `flowmap:edges` (`edge-verify --strict`) | 4 advisory `ctx.hooks` edges are audited, not proven; allowlist is the trust boundary |
...
| CLM-013 | GATE | SessionStart hook `flowmap:onboard` | harness-dependent; matcher only `startup\|resume`; no headless/subagent scoping; exit-1 handling is the harness's |
| CLM-014 | GATE | CI `flowmap:handoff:check` | dirty-handoff early exit 0; freshness = committer `%ct` (settable); strict `>` so same-commit tie passes; git error → exit 0 |
| CLM-015 | GATE | CI `loop-e2e.test` | tests the chain on a fixture/`public/plan.json`, not arbitrary plans |
| CLM-016 | CONVENTION | — | `flowmap:tooling:verify` + `tooling-map.test` in NEITHER CI nor hooks — local-only |
| CLM-017 | CONVENTION | — | "every claim is a command" not machine-checked; H5 `checkContentClaims` covers one narrow direction only |
| CLM-018 | GATE | CI `flowmap:lint` | warnings never fail; heuristic thresholds; scoped to `_bundle.mmd` |
...
| CLM-030 | GATE | CI `flowmap:acceptance` / `acceptance.test` | pure-fn only (as CLM-004) |
| CLM-031 | GATE | CI `flowmap:gate` (extract feeds gate) | as CLM-003 |
| CLM-032 | GATE | CI `spec:test` (`pipeline.test`) | test-bounded |
| CLM-033 | GATE | CI `flowmap:gate` | as CLM-003 |
| CLM-034 | GATE | CI `flowmap:cert` (apply→stubs→tsc→gate) + `build` tsc | cert scoped to `public/plan.json`; tsc-noEmit in `build` not in `spec-gate` |
| CLM-035 | GATE | CI `normtype.test` | normalizer-bounded; 1 prose type hole remains |
| CLM-036 | GATE | CI `flowmap:lint` | as CLM-018 (claim is about `--init` output failing lint) |
| CLM-037 | GATE | PreToolUse hook (contract-gate, DENY exit 2) | ONLY denies sentinel-present-but-unresolvable; see CLM-038 |
| CLM-038 | GATE(scope-limit) | — (documents CLM-037's bypass) | fail-open: malformed stdin, sentinel typo `FLOWMAP_CONTRACT`, sentinel absent, non-Agent tools, main-agent Edit/Write — all ALLOW |
| CLM-039 | GATE | CI `contract.test` + `replay.test` | determinism proven for the packet emitter, not the change interior |
| CLM-040 | GATE | CI `flowmap:coverage` | file-level; a symbol inside a covered file isn't file-gated (that's CLM-042) |
| CLM-041 | GATE | CI `flowmap:edges` | as CLM-010 |
| CLM-042 | GATE | CI `flowmap:exports` | top-level exports only; API-surface members not symbol-gated (known A1 gap) |
| CLM-043 | GATE | CI `flowmap:lint` | as CLM-018 |
...
| CLM-047 | GATE | SessionStart hook `flowmap:onboard` (exit 1) | runs `flowmap:verify`; harness decides whether exit 1 blocks; see CLM-013 |
| CLM-048 | CONVENTION | — | `orchestrate.test` IS in CI (tool behavior tested), but running the orchestrator is not forced |
| CLM-049 | GATE | CI `flowmap:cert -- --plan public/plan.json` | scoped to that one plan; `PASS_UNPROVEN`-style shape passes cert |
| CLM-050 | GATE | CI `flowmap:plan-check -- --plan public/plan.json` | scoped to that one plan |
...
| CLM-053 | GATE(audit)/CONVENTION(status) | CI `flowmap:roadmap:audit` | audit half = CLM-007 bypass; status half = CLM-006 (never blocks) |
...
| CLM-057 | GATE | CI `flowmap:validate` | grammar/structure only |
...
| CLM-070 | CONVENTION | — | `flowmap:ship` local-only; its 7 constituent gates ARE separately GATE in CI (CLM-003/010/018/040/042/057 + bundle-freshness CLM-072). The banner is an echo |
| CLM-071 | CONVENTION | — | `flowmap:tooling:verify` local-only |
| CLM-072 | GATE | CI `git diff --exit-code _bundle.mmd` (flowmap-drift) | exists ONLY in CI; no local gate fails on a stale committed bundle |
| CLM-073 | GATE | CI `flowmap:cert` | as CLM-049 |
...
6. **CLM-070 ship banner "in sync with code … edges code-backed"** — accurate about the gates it
   chains, but `flowmap:ship` is local-only; the guarantee lives in CI's separate steps, not the
   banner.
