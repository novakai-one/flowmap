...
test('coverage.mjs DENY: an uncovered source file exits 1; covering it exits 0 (F-11)', () => {
  ...
  try {
    ...
    const bad = cli('tools/flowmap/coverage.mjs', ['--src', src, '--root', root]);
    ...
    const good = cli('tools/flowmap/coverage.mjs', ['--src', src, '--root', root]);
...
test('exports-coverage.mjs DENY: a hidden export exits 1; allowlisting it exits 0 (F-11)', () => {
  ...
  try {
    ...
    const bad = cli('tools/flowmap/exports-coverage.mjs', args);
    ...
    const good = cli('tools/flowmap/exports-coverage.mjs', args);
...
test('validate.mjs DENY: a grammar error exits 1; a valid map exits 0; no arg exits 2 (F-11)', () => {
  ...
  try {
    ...
    const rBad = cli('tools/flowmap/validate.mjs', [bad]);
    ...
    const rGood = cli('tools/flowmap/validate.mjs', [good]);
    assert.equal(rGood.status, 0, `valid map must exit 0:\n${rGood.stdout}${rGood.stderr}`);
    assert.equal(cli('tools/flowmap/validate.mjs', []).status, 2, 'no arg is a usage error (2)');
  } finally { rmSync(dir, { recursive: true, force: true }); }
});
